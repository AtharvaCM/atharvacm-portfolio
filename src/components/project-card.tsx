...
export function ProjectCard({ project }: Props) {
  ...
  return (
    <article className="panel group h-full overflow-hidden p-4 transition duration-200 hover:border-border/90 hover:shadow-[0_26px_56px_-40px_hsl(var(--text)/0.18)] md:p-6">
      <ProjectCoverFrame
        alt={project.title}
      ...
      <h3 className="max-w-[15ch] font-display text-[1.75rem] leading-[0.98] tracking-tight md:text-[2rem] md:leading-[0.95]">
        <Link className="link-display text-[inherit]" href={`/projects/${project.slug}`}>
          {project.title}
        </Link>
      </h3>

      <p className="mt-4 text-sm leading-7 text-text/76 md:text-text/70">{project.excerpt}</p>
